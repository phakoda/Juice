...
RUNTIME_CORE_PATCH="$ROOT/patches/fex-runtime-core.patch"
...
# The base is disjoint from subsequent allocator/core overlays. All overlapping
...
applied="$(python3 "$ROOT/scripts/verify-patch-stack.py" "$SOURCE" "$PATCH" --optional \
  "$STIKDEBUG_PATCH" "$LIFECYCLE_PATCH" "$VALIDATION_PATCH" "$RUNTIME_CORE_PATCH" --applied-count)"
patches=("$STIKDEBUG_PATCH" "$LIFECYCLE_PATCH" "$VALIDATION_PATCH" "$RUNTIME_CORE_PATCH")
...
echo "JUICE_FEX_SOURCE_OK path=$SOURCE revision=$JUICE_FEX_REVISION stikdebug_jit=1 lifecycle=1 validation=1 runtime_core=1"

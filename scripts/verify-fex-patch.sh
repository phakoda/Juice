...
python3 "$ROOT/scripts/verify-patch-stack.py" "$SOURCE" \
  ...
  "$ROOT/patches/fex-runtime-core.patch"
python3 "$ROOT/scripts/verify-patch-stack.py" "$RPMALLOC_SOURCE" "$ROOT/patches/fex-rpmalloc-juice-ios.patch"
echo "JUICE_FEX_PATCH_OK revision=$JUICE_FEX_REVISION rpmalloc_revision=$JUICE_FEX_RPMALLOC_REVISION isolated=1 runtime_core=1"

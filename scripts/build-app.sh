...
"$CC" "${target_flags[@]}" -fobjc-arc -fblocks -O2 \
  ...
  "$ROOT/app/JuiceHardwareKeyboard.m" "$ROOT/app/JuicePointerInput.m" \
  "$ROOT/app/JuiceTextInputHardening.m" "$ROOT/app/JuiceMSIImport.m" \
  "$ROOT/app/JuiceScriptImport.m" "$ROOT/app/JuiceLogExport.m" \
  ...
  "$ROOT/app/JuiceMemoryPressure.m" "$ROOT/app/JuiceAppLifecycle.m" \

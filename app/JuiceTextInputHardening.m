...
static BOOL JuiceSendTextPayload(id self, NSData *payload, uint64_t hwnd,
                                 NSUInteger *chunkCount)
{
    if (!payload.length || !hwnd) return NO;
    ...
    while (offset < payload.length)
    {
        ...
        /* UTF-16LE code units are two bytes; keep every wire payload aligned. */
        length &= ~(NSUInteger)1;
        if (!length) return NO;
        ...
        BOOL delivered = ((BOOL (*)(id, SEL, JuiceTextMsg *, id))objc_msgSend)
            (self, selector, &message, part);
        if (!delivered) return NO;
        offset += length;
        chunks++;
    }
...
static BOOL JuiceSendText(id self, NSString *text, NSString *source)
{
    ...
    NSUInteger chunks = 0;
    BOOL delivered = JuiceSendTextPayload(self, payload, hwnd, &chunks);
    JuiceTextAppend(self, [NSString stringWithFormat:
...
static void JuicePasteClipboard(id self, SEL _cmd, UIButton *sender)
{
    (void)_cmd;
    (void)sender;
...
static void JuiceTextViewDidLoad(id self, SEL _cmd)
{
    ...
    [paste addTarget:self action:@selector(juice_pasteIOSClipboard:)
...
static void JuiceInstallTextInputHardening(void)
{
    ...
    SEL pasteSelector = NSSelectorFromString(@"juice_pasteIOSClipboard:");
    class_addMethod(cls, pasteSelector, (IMP)JuicePasteClipboard, "v@:@");

    Method send = class_getInstanceMethod(cls, NSSelectorFromString(@"sendGuiTextTapped"));
    ...
    if (view)
        JuiceOriginalTextViewDidLoad = (void (*)(id, SEL))
            method_setImplementation(view, (IMP)JuiceTextViewDidLoad);
}
